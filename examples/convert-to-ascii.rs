...
fn main() {
    ...
    let mut emitter = EmitterConfig::new().fbx_version(Some(7500)).create_writer(File::create(new_filename.clone()).unwrap());
    let mut depth = 0;
    for e in parser {
        match e {
        ...
        match e {
            Ok(ReaderEvent::StartFbx(_)) => {
                emitter.write(WriterEvent::StartFbx(fbx_direct::common::FbxFormatType::Ascii)).unwrap();
            },
            Ok(ref e) => {
